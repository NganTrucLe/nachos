...
{
    if (id == 0)
        this->parentID = -1; //tien trinh chay dau tien nen parentID = 0
    else
        this->parentID = currentThread->processID; //gan ID cha cho tien trinh hien tai

	this->numwait = this->exitcode = this->boolBG = 0;
	this->thread = NULL;

	this->joinsem = new Semaphore("joinsem",0); //bien thuc hien theo thu tu thi la 0 con khong la 1
...
{
	multex->P();
	++numwait; //tai thoi diem nay thi CPU chi thuc hien 1 (doc quyen)
	multex->V(); 
}

void PCB::DecNumWait()
{
	multex->P(); //kep giua P va V
	if(numwait > 0)
		--numwait;
	multex->V();
}
...
{  
	...
	this->thread = new Thread(filename); //Trong Thread.h
