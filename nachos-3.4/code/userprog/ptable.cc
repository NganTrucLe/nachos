...
PTable::PTable(int size)
{
    ...
    psize = size;
    bm = new BitMap(size); //tao ra bitmap luu dc so bit theo int size
	...
	bm->Mark(0); //danh dau 0 da su dung

	pcb[0] = new PCB(0); //nap cho tien trinh cha
...
int PTable::ExecUpdate(char* name)
{
	...
	//nap tien trinh vao bang PCB de quan ly
	...
	return pid; //tra ve o duoc cap phat
}
...
int PTable::JoinUpdate(int id)
{
	...
	if(id < 0) //kiem tra xem co nam ngoai bang ko
	{
	...
	pcb[pcb[id]->parentID]->IncNumWait(); //cha dang chay ma con chay vo thi tang tien trinh
	

	//pcb[id]->boolBG = 1;
	
	pcb[id]->JoinWait(); //cho tien trinh cha wait
...
{              
	...
	if(id == 0)
	{
		...
		currentThread->FreeSpace(); // = 0 thi free va tat may	
		interrupt->Halt();
		return 0;
	}
    
        if(IsExist(id) == false) //kiem tra processID co ton tai hay khong?
	{
	...
	pcb[pcb[id]->parentID]->DecNumWait(); //neu thoat roi thi giam so tien trinh cua cha di 1
    
    // Gọi JoinRelease để giải phóng tiến trình cha đang đợi nó(nếu có) và ExitWait() để xin tiến trình cha
    // cho phép thoát.
	pcb[id]->JoinRelease();
    // 
	pcb[id]->ExitWait(); // doi tien trinh cha thoat
