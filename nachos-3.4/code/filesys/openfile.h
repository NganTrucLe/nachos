...
class OpenFile {
  public:
    ...
    OpenFile(char *name, int f) {strncpy(filename, name, MAX_LENGTH); file = f; currentOffset = 0; opened=TRUE;}	// open the file
    OpenFile(char* name, int f, int t) {strncpy(filename, name, MAX_LENGTH);  file = f; currentOffset = 0; type=t; opened=TRUE;}
    ~OpenFile() { opened=FALSE ;Close(file); }			// close the file
...
/*
    ...
    type 3: stdout
*/
    OpenFile(char* name, int sector);		// Open a file whose header is located
					// at "sector" on the disk
    OpenFile(char* , int sector, int type);
